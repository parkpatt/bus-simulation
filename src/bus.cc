 ...
 *
 * @brief Contains method definitions for Bus class
 */
#include "./bus.h"
...
bool Bus::IsTripComplete() {
  ...
  // std::cout << "outgoing: " << outgoing_route_->IsAtEnd() << std::endl;
  // std::cout << "incoming: " << incoming_route_->IsAtEnd() << std::endl;
...
bool Bus::Move() {
  ...
  if (distance_remaining_ <= 0) {
	...
	std::list<Passenger* > passengers = GetPassengerList();
	bool unloadAtNextStop = false;
	for (auto* pass : passengers){
	  if (pass->GetDestination() == nextStop->GetId()){
	    unloadAtNextStop = true;
	    break;
	  }
	}
	//  Only load/unload if passengers present at next stop or
	//  at least one passenger on bus needs to get off
	if (nextStop->GetNumPassengersPresent() > 0 || unloadAtNextStop){
	  passengers_handled += UnloadPassengers();  // unload
	  passengers_handled += next_stop_->LoadPassengers(this);  // load

	  // if any passengers on or off, all distance to next stop is lef
	  // but, if we didn't handle any passengers here, any negative 
	  // will affect the distance remaining (see addition below)
	}
    ...
    //  only load/unload if passengers present at next stop or at least
    //  one passenger on bus needs to get off
    if (nextStop->GetNumPassengersPresent() > 0 || unloadAtNextStop){
      passengers_handled += UnloadPassengers();  // unload
      passengers_handled += next_stop_->LoadPassengers(this);  // load
    }
...
void Bus::UpdateBusData() {
  ...
  p.y =
    (nextStop->GetLatitude() * (1 - ratio) + prevStop->GetLatitude() * ratio);
